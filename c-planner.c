...
// hours, repeating for the total of the types of duty. The input buffer is flushed each time user inputs data and enters a
// space right after to prevent the next scanf from "skipping" due to the buffer containing the "\n".
int get_duties(void)
{
    int total_duties;
    printf("How many duties?:");
    scanf("%d", &total_duties);
    int ch;
    while ((ch = getchar()) != '\n' && ch != EOF);
    ...
    {
        printf("Duty Name:");
        scanf("%s", all_duty_types[i]);
        while ((ch = getchar()) != '\n' && ch != EOF);
        printf("Total hours for this duty:");
        scanf("%d", &all_duty_hours[i]);
        while ((ch = getchar()) != '\n' && ch != EOF);
    }
...
// get_strength() gets the total number of people from the user and returns this value to main(). The input buffer is flushed
// each time user inputs data and enters a space right after to prevent the next scanf from "skipping" due to the buffer
// containing the "\n".
int get_strength(void)
{
    int strength;
...
// get_names() using the strength previously asked by the user, asks the user for each name part of this strength. The
// input buffer is flushed each time user inputs data and enters space right after to prevent the next scanf from
// "skipping" due to the buffer containing the "\n".
void get_names(int strength, individual_schedule whole_schedule[])
{
    int ch;
...
// This for-loop assigns a default value to each slot in the array to prevent segmentation faults from occuring.
    for (int a = 0; a < strength; a++)
    {
        for(int b = 0; b < i_length; b++)
        {
            ...
            strcpy(whole_schedule[a].all_duty_blocks[b].duty_type, "BREAK");
        }
    }
...
// total duty_hours for this respective duty is subtracted by one.
    for (int i = 0; i < 4; i++)
    {
        ...
        strcpy(whole_schedule[random].all_duty_blocks[0].duty_type, all_duty_types[random_duty]);
...
// total duty_hours for this respective duty is subtracted by one.
    for (int j = 0; j < 4; j++)
    {
        ...
        strcpy(whole_schedule[random].all_duty_blocks[3].duty_type, all_duty_types[0]);
        whole_schedule[random].all_duty_blocks[3].is_duty = true;
        all_duty_hours[0] --;

        strcpy(whole_schedule[random].all_duty_blocks[4].duty_type, all_duty_types[0]);
...
// total duty_hours for this respective duty is subtracted by one.
    for (int k = 0; k < 4; k++)
    {
        ...
        strcpy(whole_schedule[random].all_duty_blocks[14].duty_type, all_duty_types[0]);
        whole_schedule[random].all_duty_blocks[14].is_duty = true;
        all_duty_hours[0] --;

        strcpy(whole_schedule[random].all_duty_blocks[15].duty_type, all_duty_types[0]);
...
// consecutive 2 hours of duty.
    for (int l = 0; l < strength; l++)
    {
        for (int p = 2; p < i_length; p++)
        {
            ...
            if ((p > 4)  && random_int(0, 1))
            {
                ...
                {
                    strcpy(whole_schedule[l].all_duty_blocks[p].duty_type, "BREAK");
                    continue;
                }
            ...
            int random_duty = random_int(0 , total_duties);
            strcpy(whole_schedule[l].all_duty_blocks[p].duty_type, all_duty_types[random_duty]);
    ...
    for (int g = 0; g < strength; g++)
    {
        ...
        {
            if (strlen(whole_schedule[g].all_duty_blocks[f].duty_type) <= 0)
            {
                 strcpy(whole_schedule[g].all_duty_blocks[f].duty_type, "BREAK");
            }
        }
    }
}
