...
void plan_schedule(int strength, individual_schedule whole_schedule[])
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (whole_schedule[random].all_duty_blocks[0].is_duty == true)
        {
            whole_schedule[random].all_duty_blocks[1].duty_type = all_duty_types[random_duty];
            whole_schedule[random].all_duty_blocks[1].is_duty = true;
    ...
    for (int l = 0; l < ws_length; l++)
    {
        for (int p = 0; p < i_length; p++)
        {
            ...
            if (((whole_schedule[l].all_duty_blocks[p - 1].is_duty)) && (( whole_schedule[l].all_duty_blocks[p - 2].is_duty)) && (random_int(0, 1)))
            {
                whole_schedule[l].all_duty_blocks[p].duty_type = "BREAK";
                continue;
            }
...
void print_schedule(int strength, individual_schedule whole_schedule[])
{
    ...
    for (int l = 0; l < ws_length; l++)
    {
        printf("%-6s", whole_schedule[l].name);
        printf("    ||");
        for (int p = 0; p < i_length; p++)
        {
